...
但是，这并不意味着引用是通过拷贝指针来实现的。c++的引用的语义是别名，没有规定说编译器怎么实现，只是编译器一般倾向都过常量指针去做。引用的实现细节可能因编译器和平台的不同而不同。
...
#### 3.2 一个通过`new`创建出来的指针，若被`delete`两次会怎样？
...
#### 3.3 为什么在 delete 之后，通常都会将指针设置为 nullptr
