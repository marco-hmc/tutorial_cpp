...
namespace {
    ...
    template <typename K, typename V,
              std::enable_if_t<std::is_same_v<std::decay_t<V>, bool>, void*> =
                  nullptr>
...
int main() {
    ...
    assert(a == kv_string("data"));
    assert(b == kv_string("data", "name", "jc", "ID", 1));
    assert(c == kv_string("data", "name", "jc", "ID", 1, "active", true));
}
