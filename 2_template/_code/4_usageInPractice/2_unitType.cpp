...
int main() {
    constexpr auto a =
        Ratio::duration<double, Ratio::ratio<1, 1000>>(10);  // 10 ms
    constexpr auto b =
        Ratio::duration<double, Ratio::ratio<1, 3>>(7.5);  // 2.5 s
    constexpr auto c = a + b;  // 10 * 3 + 7.5 * 1000 = 7530 * 1/3000 s
    static_assert(std::abs(c.count() - 7530) < 1e-6);
    static_assert(
        std::is_same_v<std::decay_t<decltype(c)>,
                       Ratio::duration<double, Ratio::ratio<1, 3000>>>);
